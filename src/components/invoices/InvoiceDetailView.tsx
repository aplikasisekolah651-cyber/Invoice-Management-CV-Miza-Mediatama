...
export const InvoiceDetailView: React.FC<InvoiceDetailViewProps> = ({
  ...
  const relatedPayments = payments.filter(
    (p) =>
      (invoice?.id && p.invoiceId === invoice.id) ||
      (invoice?.invoiceNumber && p.invoiceNumber === invoice.invoiceNumber)
  );
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-2 text-xs">
          ...
          {(invoice.customerSnapshot?.name || invoice.customerSnapshot?.contactPerson) && (
            <div className="text-slate-700 font-semibold text-xs flex items-center gap-1">
              <span className="text-slate-500 font-normal">Nama Kontak Person:</span>
              <span className="text-blue-700 font-bold">{invoice.customerSnapshot?.name || invoice.customerSnapshot?.contactPerson}</span>
            </div>
          )}
                  ...
                  <tr key={p.id} className="hover:bg-slate-50">
                    <td className="py-2.5 px-3 font-mono font-bold text-slate-900">
                      <button
                        ...
                        className="hover:underline text-emerald-700 cursor-pointer font-mono font-bold"
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <button
                        onClick={() => setSelectedReceipt(p)}
                        className="px-2.5 py-1 text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs font-semibold transition-colors cursor-pointer inline-flex items-center gap-1"
                        title="Lihat Kuitansi Sah"
                      >
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 text-xs space-y-4">
            ...
            <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-emerald-950">{selectedReceipt.customerName || invoice.customerSnapshot?.companyName || invoice.customerSnapshot?.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-emerald-900">Untuk Pembayaran:</span>
                <span className="font-semibold text-emerald-950">Invoice {selectedReceipt.invoiceNumber || invoice.invoiceNumber}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="text-[11px] text-slate-500 text-center">
              Diterima dan dicatat resmi oleh {selectedReceipt.createdByName || 'Staff Keuangan'}
            </div>
